  ...
  navset_card_tab(
    nav_panel("Description", p("Project CannTalk is a longitudinal observational study tracking habitual cannabis users over the course of approximately 9 months. 
                            ...
                            p("App Usage: Choose an enrollment stage (up to Total Enrollment), a Category (e.g. Age Group), and any tests to perform.The term 'washout' refers to: Lost to follow up, Ineligible, Not Interested(withdrew).
                                  The majority of these were Lost.")),
...
#this reactive data frame removes NA values, and feeds into (1) summary data in the next step, and (2) into the contingency table.
  filter_data <- eventReactive(input$action, {CT_washout %>% drop_na(all_of(input$status))}) 
...
#simple faceted plot that displays the various categories. Some aspects such as the legend or labels on the x-axis need to be cleaned up
  output$plot1 <- renderPlot({
    ggplot(data(), aes(x = .data[[input$status]], y = count, fill = .data[[input$factor]]))+
